...
void NameNode::init(const string &extent_dst, const string &lock_dst) {
  ...
  heartcut = 0;
  NewThread(this, &NameNode::countbeating);
  /* Add your init logic here */
}

void NameNode::countbeating(){
  while(true){
    this->heartcut++;
    sleep(1);
  }
...
list<NameNode::LocatedBlock> NameNode::GetBlockLocations(yfs_client::inum ino) {
  ...
  for(auto item : block_ids){
    ...
    #endif
    i++;
    LocatedBlock lb(item,  size1, i < block_ids.size() ? BLOCK_SIZE : (attr.size -  size1), GetDatanodes());
    l.push_back(lb);
     size1 += BLOCK_SIZE;
  }
  return l;
...
void NameNode::DatanodeHeartbeat(DatanodeIDProto id) {
  int m = 0;
  for (auto i : datanodesmap){
    m = max(m, i.second);
  }
  datanodesmap[id] = this->heartcut;
}

void NameNode::RegisterDatanode(DatanodeIDProto id) {
  if (this->heartcut > 5){
    #if DB 
    cout << "recovery !!!!!!" << datanodesmap.size() << endl;
    cout.flush();
    #endif
    for(auto b : modified_blocksset){
      ReplicateBlock(b, master_datanode, id);
    }
  }
  datanodesmap.insert(make_pair(id, this->heartcut));
}

list<DatanodeIDProto> NameNode::GetDatanodes() {
  list<DatanodeIDProto> l;
  for (auto i : datanodesmap){
    if (i.second >= this->heartcut - 3){
      l.push_back(i.first);
    }
  }
  return l;
